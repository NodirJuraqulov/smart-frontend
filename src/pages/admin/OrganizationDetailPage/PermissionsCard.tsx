...
export default function PermissionsCard({ orgId }: PermissionsCardProps) {
  ...
  const mutation = useMutation({
    mutationFn: (permissions: OperatorPermissions) =>
      updatePermissions({ id: orgId, permissions }),
    onSuccess: () => {
      message.success(t('permissions.updateSuccess'))
      setIsEditing(false)
      queryClient.invalidateQueries({ queryKey: ['permissions', orgId] })
    },
    onError: (error) => {
  ...
  return (
    <Card variant="borderless" title={t('permissions.title')}>
